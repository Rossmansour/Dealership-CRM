...
- **Taxes & Fees: Admin-Configurable Reference Table (CA and AZ)**
- Rebuilt from a hardcoded formula into how real DMS platforms actually do it: a **🗺️ Taxes & Fees** admin screen (Deals tab) where sales tax rates are set up once per State / County / City, with separate State/County/City percentages that sum to the combined rate -- visible and editable, not buried in code
