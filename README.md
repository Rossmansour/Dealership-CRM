...
- **Automatic Sales Tax & DMV Fees (CA and AZ)**
- One click ("🧮 Auto-Calculate Tax & DMV Fees" on the Deal Terms panel) calculates sales tax rate and DMV-style fees from the deal's **State** field, the customer's ZIP (from their credit application), the vehicle's price, and its model year -- not guessed, not flat, actually different per state:
  - **California**: taxes the *full* vehicle price -- trade-in does **not** reduce the taxable amount (Rev. & Tax. Code §6012), unlike most states. DMV fees: $74 base + $29 CHP fee + a value-tiered Transportation Improvement Fee, a $28 title fee, and the Vehicle License Fee (0.65% of price) mapped onto the License Fee field, since VLF literally *is* California's license fee.
  - **Arizona**: trade-in *does* reduce the taxable amount (the standard rule). DMV fees: a genuinely age-dependent Vehicle License Tax -- 60% of price in year one, depreciating 16.25% per year after that, taxed at 2.80% (new) or 2.89% (used/renewal) -- calculated from the vehicle's actual model year, not a flat guess.
  - **Sales tax rate** for both states is approximated from published county/city reference rates via ZIP-code prefix (a curated set of major metro areas, not a live per-address CDTFA/ADOR lookup), falling back to each state's statewide base rate for unlisted ZIPs.
  - **Any other state** falls back to California's numbers, clearly labeled as a fallback (e.g., `"CA (fallback -- TX not yet built)"`) rather than a silent, unlabeled guess -- more states can be added the same way later.
- **County field, auto-filled from ZIP**: the customer's county now auto-populates the moment you tab out of the ZIP field on the Credit Application (CA and AZ only, same coverage as the rest of this feature) -- shown and editable, not just an invisible lookup key, so a sales rep can correct it if a ZIP falls near a county boundary and the auto-fill guesses wrong
- **Fixed a real bug**: "Auto-Calculate" was reading the customer's ZIP from the *last saved* credit application, not what's currently typed on screen -- editing an address and clicking Auto-Calculate without saving first used stale (sometimes empty) data. It now reads the live form fields directly.
- This is a genuine calculation engine, not a lookup table copy -- verified by hand against the actual published CA and AZ formulas (VLF, TIF brackets, VLT depreciation) before shipping.
